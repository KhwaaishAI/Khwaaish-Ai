...
"""
    ...
    async def search(self, query, save_html=False, save_json=True):
        """
        Search Flipkart for products and extract information
        
        Args:
            query: Search query string or full search URL
        ...
        """
        # Check if query is already a full URL
        if query.startswith('http'):
            ...
            query_term = query_params.get('q', ['unknown'])[0]
        else:
            search_url = f"{self.base_url}/search?q={query}"
            query_term = query
        
        # Sanitize filename (remove invalid characters)
        safe_filename = re.sub(r'[<>:"/\\|?*]', '_', query_term)
        ...
        async with AsyncWebCrawler() as crawler:
            ...
            if save_html:
                with open(f"flipkart_search_{safe_filename}.html", "w", encoding="utf-8") as f:
                    f.write(html)
                print(f"✓ Saved HTML to flipkart_search_{safe_filename}.html")
            ...
            if save_json:
                with open(f"app/agents/flipkart/Extra/flipkart_{safe_filename}.json", 'w', encoding='utf-8') as f:
                    json.dump(products, f, ensure_ascii=False, indent=2)
                print(f"✓ Saved {len(products)} products to flipkart_{safe_filename}.json")
    ...
    def search_sync(self, query, save_html=False, save_json=True):
        """
        Synchronous wrapper for search method
        
        Args:
            query: Search query string
            save_html: Whether to save extracted HTML (default: False)
            save_json: Whether to save extracted JSON (default: True)
